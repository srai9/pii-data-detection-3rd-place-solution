...
def train(fold):
    ...
    run = wandb.init(project = cfg.project,
                   name=f'{model_dir.split("/")[-1]}_fold_{fold}',
                   group=f'{model_dir.split("/")[-1]}',
                   job_type="training",
                 #  config=cfg.to_dict()
                    )
    ...
    ext_data = pd.read_json('../datasets/mixtral-8x7b-v1.json')
    ...
    print('Train before adding external data', train_df.shape)
    train_df = pd.concat([train_df, ext_data])
    print('Train after adding external data', train_df.shape)
